	...
	export default {
		...
		methods: {
			...
			initParams() {
				...
				if (this.$mp.query.auth_code) {
					this.code = this.$mp.query.auth_code
				}
				if (this.$mp.query.code) {
					this.code = this.$mp.query.code
				}
				if (this.$mp.query.order_id) {
					this.orderId = this.$mp.query.order_id
				}
				if (this.$mp.query.operator_id) {
					this.operatorId = this.$mp.query.operator_id
				}
				// #endif
				// #ifndef MP-WEIXIN
			...
			simpleInfo(){
				this.disabled = true
				console.log(this.user_id, this.order_id, this.operator_id);
				this.$u.api.institution.trade.SimpleInfo({ 
					userId: this.user_id,
					orderId: this.order_id,
					qrcodeId: this.operator_id,
				}).then(res=>{
					this.disabled = false
