<template>
	<view class="u-keyboard" @touchmove.stop.prevent="() => {}">
		...
		<view class="u-keyboard-right">
			<view class="u-keyboard-grids-item u-border-left u-bg-gray" hover-class="u-hover-class" :hover-stay-time="100" @touchstart.stop="backspaceClick"
				@touchend="clearTimer">
				<view class="u-keyboard-back u-keyboard-grids-btn">
					<u-icon name="backspace" :size="38" :bold="true"></u-icon>
				</view>
			</view>
			<view v-if="confirmBtn" class="u-keyboard-grids-item u-border-left u-bg-confirm" hover-class="u-hover-class" :hover-stay-time="100" @touchstart.stop="onConfirm"
				@touchend="clearTimer">
				<view class="u-keyboard-confirm u-keyboard-grids-btn">
					{{confirmText}}
				</view>
			</view>
		</view>
		...
		computed: {
			...
			numList() {
				...
				if (!this.dotEnabled && this.mode == 'number') {
					...
					if (!this.random) {
						return [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, this.dot];
					} else {
						return this.$u.randomArray([1, 2, 3, 4, 5, 6, 7, 8, 9, 0, this.dot]);
					}
				} else if (this.mode == 'card') {
					if (!this.random) {
						return [1, 2, 3, 4, 5, 6, 7, 8, 9, this.cardX, 0];
					} else {
						return this.$u.randomArray([1, 2, 3, 4, 5, 6, 7, 8, 9, 0], this.cardX);
					}
				}
			},
			...
			itemStyle() {
				return index => {
					...
					if (this.mode == 'number' && !this.dotEnabled && index == 9) style.flex = '0 0 100%';
					return style;
				};
			},
			...
			btnBgGray() {
				return index => {
					if (!this.random && index == 10 && (this.mode != 'number' || (this.mode == 'number' && this.dotEnabled))) return true;
					else return false;
				};
			},
			hoverClass() {
